...
def _convert_layers(layers: Iterable) -> tuple[tuple[tuple[str, ...], ...], ...]:
    ...
    return tuple(
        tuple(tuple(band_nodes) for band_nodes in layer_bands) for layer_bands in layers
    )
...
class Ordering(object):
    ...
    def __repr__(self):
        ...
        return "Ordering( {} )".format(
            " | ".join(format_layer(layer) for layer in self.layers)
        )
    ...
    def insert(self, i, j, k, value):
        ...
        def _insert(layer):
            return [
                __insert(band) if j == jj else band for jj, band in enumerate(layer)
            ]

        layers = [
            _insert(layer) if i == ii else layer for ii, layer in enumerate(self.layers)
        ]
    ...
    def remove(self, value):
        ...
        layers = tuple(layer for layer in layers if any(rank for rank in layer))
...
def new_node_indices(G, this_bands, other_bands, new_process_group, side="below"):
    ...
    existing_pos = [
        median_value(neighbour_positions(G, other_layer, u)) for u in this_layer
    ]
    ...
    index = (
        bisect.bisect_right(candidates, new_pos)
        if side == "below"
        else bisect.bisect_left(candidates, new_pos)
    )
