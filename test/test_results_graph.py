...
def test_results_graph_overall():
    ...
    view_graph.add_node("c", node=ProcessGroup([], c_partition))
    ...
    assert sorted(Gr.edges(keys=True, data=True)) == [
        (
            ...
            "via^m",
            ("m", "*"),
            {"measures": {"value": 3}, "original_flows": [0], "bundles": [0]},
        ),
        (
            "a^*",
            "via^n",
            ("n", "*"),
            {"measures": {"value": 1}, "original_flows": [1], "bundles": [0]},
        ),
        (
            "b^*",
            "via^m",
            ("m", "*"),
            {"measures": {"value": 3}, "original_flows": [2, 3], "bundles": [1]},
        ),
        (
            "b^*",
            "via^n",
            ("n", "*"),
            {"measures": {"value": 1}, "original_flows": [4], "bundles": [1]},
        ),
        (
            "via^m",
            "c^c1",
            ("m", "*"),
            {"measures": {"value": 4}, "original_flows": [0, 2], "bundles": [0, 1]},
        ),
        (
            "via^m",
            "c^c2",
            ("m", "*"),
            {"measures": {"value": 2}, "original_flows": [3], "bundles": [0, 1]},
        ),
        (
            "via^n",
            "c^c1",
            ("n", "*"),
            {"measures": {"value": 1}, "original_flows": [1], "bundles": [0, 1]},
        ),
        (
            "via^n",
            "c^c2",
            ("n", "*"),
            {"measures": {"value": 1}, "original_flows": [4], "bundles": [0, 1]},
        ),
    ]
    ...
    # title
    assert groups == [
        {"id": "via", "title": "", "type": "group", "nodes": ["via^m", "via^n"]},
        {"id": "c", "title": "", "type": "process", "nodes": ["c^c1", "c^c2"]},
    ]
...
def test_results_graph_time_partition():
    ...
    # Do partition based on flows stored in bundles
    Gr, groups = results_graph(view_graph, bundle_flows, time_partition=time_partition)
    assert sorted(Gr.edges(keys=True, data=True)) == [
        (
            "a^*",
            "b^*",
            ("*", "1"),
            {"measures": {"value": 6}, "original_flows": [0, 1, 2], "bundles": [0]},
        ),
        (
            "a^*",
            "b^*",
            ("*", "2"),
            {"measures": {"value": 4}, "original_flows": [3, 4], "bundles": [0]},
        ),
    ]
    ...
    Gr, groups = results_graph(
        view_graph, bundle_flows, material_partition, time_partition
    )
    assert sorted(Gr.edges(keys=True, data=True)) == [
        (
            "a^*",
        ...
        (
            ...
            "b^*",
            ("m", "2"),
            {"measures": {"value": 1}, "original_flows": [3], "bundles": [0]},
        ),
        (
            "a^*",
            "b^*",
            ("n", "1"),
            {"measures": {"value": 3}, "original_flows": [1, 2], "bundles": [0]},
        ),
        (
            "a^*",
            "b^*",
            ("n", "2"),
            {"measures": {"value": 3}, "original_flows": [4], "bundles": [0]},
        ),
    ]
...
def test_results_graph_material_key():
    ...
    assert sorted(Gr.edges(keys=True, data=True)) == [
        (
            ...
            "c^*",
            ("long", "*"),
            {"measures": {"value": 4}, "original_flows": [0, 1], "bundles": [0]},
        ),
    ]
...
def test_results_graph_measures():
    ...
    assert list(Gr.edges(keys=True, data=True)) == [
        (
            ...
            "b^*",
            ("*", "*"),
            {"measures": {"value": 11}, "original_flows": [0, 1], "bundles": [0]},
        ),
    ]

    # Results using measure = 'another_measure'
    Gr, groups = results_graph(view_graph, bundle_flows, measures="another_measure")
    assert list(Gr.edges(keys=True, data=True)) == [
        (
            "a^*",
            "b^*",
    ...
    Gr, groups = results_graph(
        view_graph, bundle_flows, measures={"value": "sum", "another_measure": "mean"}
    )
    assert list(Gr.edges(keys=True, data=True)) == [
        (
            "a^*",
            ...
            ("*", "*"),
            {
                "measures": {"value": 11, "another_measure": 1.5},
                "original_flows": [0, 1],
                "bundles": [0],
            },
        ),
...
def test_results_graph_unused_nodes():
    ...
    view_graph = LayeredGraph()
    view_graph.add_node("a", node=ProcessGroup([], partition_a))
    view_graph.add_node("b", node=ProcessGroup([], partition_b))
    view_graph.add_edges_from(
        [
            ("a", "b", {"bundles": [0]}),
        ]
...
def test_results_graph_with_extra_or_not_enough_groups():
    ...
    node_a = ProcessGroup([], Partition.Simple("process", ["a1", "a3"]))
    node_b = ProcessGroup([], Partition.Simple("process", ["b1"]))
    view_graph = LayeredGraph()
...
def test_results_graph_elsewhere_stubs():
    ...
    view_graph.add_node("a", node=ProcessGroup([]))
    view_graph.add_node(
        "b", node=ProcessGroup([], b_partition), from_elsewhere_bundles=[1]
    )
    ...
    assert sorted(Gr.nodes(data=True)) == [
        ("a^*", {"direction": "R", "type": "process", "title": "a"}),
        (
            "b^b1",
            {
                "direction": "R",
