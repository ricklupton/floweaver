...
from floweaver.sankey_definition import (
    SankeyDefinition,
    Ordering,
    ProcessGroup,
    Waypoint,
...
def test_view_graph_bundle_flow_partitions_must_be_equal():
    ...
    with pytest.raises(ValueError):
        view_graph(SankeyDefinition(nodes, bundles, order))

    bundles[1] = Bundle(
        "b", "c", waypoints=["via"], flow_partition=material_partition_mn
    )
