...
def sankey_data_equivalent(new_result, old_result):
    ...
    def _link_props(link):
        return (link.source, link.target, link.type, link.time, link.title, link.color, set(link.original_flows))
    assert (
        sorted([_link_props(link) for link in new_result.links]) ==
        sorted([_link_props(link) for link in old_result.links])
    )
    ...
    assert set(old_links_dict.keys()) == set(new_links_dict.keys()), "Link keys don't match"
...
def assert_node_directions(old_result, new_result):
    ...
    old_dirs = {n.id: n.direction for n in old_result.nodes}
    new_dirs = {n.id: n.direction for n in new_result.nodes}
    assert new_dirs == old_dirs


def assert_link_endpoints(old_result, new_result):
    """Compare link source/target pairs."""
    old_endpoints = sorted((l.source, l.target) for l in old_result.links)
    new_endpoints = sorted((l.source, l.target) for l in new_result.links)
...
def assert_link_values(old_result, new_result, measure='value'):
    ...
    old_vals = {(l.source, l.target, l.type, l.time): l.data.get(measure)
                for l in old_result.links}
    new_vals = {(l.source, l.target, l.type, l.time): l.data.get(measure)
                for l in new_result.links}
    assert new_vals == old_vals


def assert_link_original_flows(old_result, new_result):
    """Compare original flow indices for each link."""
    old_flows = {(l.source, l.target, l.type, l.time): sorted(l.original_flows)
                 for l in old_result.links}
    new_flows = {(l.source, l.target, l.type, l.time): sorted(l.original_flows)
                 for l in new_result.links}
...
def assert_group_nodes(old_result, new_result):
    ...
    old_groups = {g['id']: set(g['nodes']) for g in old_result.groups}
    new_groups = {g['id']: set(g['nodes']) for g in new_result.groups}
    assert new_groups == old_groups
...
class TestColorScaleEquivalence:
    ...
    def test_categorical_color_by_type(self):
        ...
        old_colors_by_type = {l.type: l.color for l in old_result.links}
        new_colors_by_type = {l.type: l.color for l in new_result.links}
...
class TestMeasureEquivalence:
    ...
    def test_multiple_measures(self):
        ...
        new_result = weave_compiled(sdd, dataset, measures=['value', 'cost'], link_width='value')
...
class TestLinkWidthEquivalence:
    ...
    def test_link_width_matches_value(self):
        ...
        old_widths = {(l.source, l.target): l.link_width for l in old_result.links}
        new_widths = {(l.source, l.target): l.link_width for l in new_result.links}
...
class TestComplexRealWorldEquivalence:
    ...
    def test_fruit_example_from_paper(self):
        ...
        new_result = weave_compiled(sdd, dataset, dimension_tables=dimension_tables, palette=palette)
        ...
        old_nodes_filtered = sorted([n for n in old_result.nodes if n.id in common_nodes])
        new_nodes_filtered = sorted([n for n in new_result.nodes if n.id in common_nodes])
        ...
        new_groups = sorted(filter_groups(new_result.groups, common_nodes), key=lambda group: group["id"])
        old_groups = sorted(filter_groups(old_result.groups, common_nodes), key=lambda group: group["id"])
        assert new_groups == old_groups

        # Check link properties (excluding floating point values)
        def _link_props(link):
            return (link.source, link.target, link.type, link.time, link.title, link.color)
        assert (
            sorted([_link_props(link) for link in new_result.links]) ==
            sorted([_link_props(link) for link in old_result.links])
        )
